<!--
  Template 4: Info LEFT, Avatar RIGHT
  Contact below, CTA + social at bottom
-->
<script lang="ts">
					...
					<tbody>
						<tr>
							...
							<td style="vertical-align:top;">
								<div style="font-size:{fs + 4}px;font-weight:700;color:{s.nameColor};">
									{fullName}
								</div>
								...
								<div style="margin-top:6px;font-size:{fs - 2}px;color:{s.mutedColor};">
									...
									{#if data.mobilePhone}<span
											>{data.officePhone ? ' · ' : ''}{data.mobilePhone}</span
										>{/if}
									{#if data.website}<br /><a
											href={data.website}
											style="color:{s.linkColor};text-decoration:none;word-break:break-word;overflow-wrap:anywhere;"
											>{data.website}</a
										>{/if}
							...
							<!-- Avatar right -->
							<td style="vertical-align:top;text-align:right;padding-left:14px;width:80px;">
								{#if data.avatarUrl}
									<img
										src={data.avatarUrl}
										alt={fullName}
										width="80"
										height="80"
										style="display:block;width:80px !important;height:80px !important;min-width:80px;min-height:80px;max-width:80px;max-height:80px;border-radius:9999px;object-fit:cover;object-position:center center;box-sizing:border-box;{s.avatarBorder
											? `border:${s.avatarBorder}`
											: ''}"
									/>
								{:else}
									<div
										style="width:80px;height:80px;border-radius:50%;background:{data.primaryColor};display:flex;align-items:center;justify-content:center;font-size:28px;font-weight:700;color:#fff;"
									>
										{initials}
									</div>
								{/if}
									...
									{:else}
										<a
											...
											style="display:inline-block;padding:7px 14px;background:{s.ctaBg};color:{s.ctaColor};border-radius:6px;text-decoration:none;font-size:{fs -
												1}px;font-weight:600;{s.ctaBorder !== 'none'
												? `border:${s.ctaBorder}`
												: ''}">{data.ctaText}</a
										>
									{/if}
						...
						{#if data.disclaimer}
							<tr>
								<td colspan="2" style="padding-top:12px;">
									<div
										style="border-top:1px solid {s.borderColor};padding-top:8px;font-size:{fs -
