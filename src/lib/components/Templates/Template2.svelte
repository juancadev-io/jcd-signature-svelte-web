...
<table
	cellpadding="0"
	cellspacing="0"
	style="font-family:{data.fontFamily};font-size:{fs}px;line-height:1.35;color:{s.textColor};background:{s.containerBg};border-radius:{s.containerRadius};width:100%;max-width:400px;"
>
	<tbody>
		<tr>
			<td style="text-align:center;">
				...
				<div style="margin-bottom:10px;">
					{#if data.avatarUrl}
						<img
							...
							width="80"
							height="80"
							style="display:block;margin:0 auto;width:80px !important;height:80px !important;min-width:80px;min-height:80px;max-width:80px;max-height:80px;border-radius:9999px;object-fit:cover;object-position:center center;box-sizing:border-box;{s.avatarBorder
								? `border:${s.avatarBorder}`
								: ''}"
						/>
					{:else}
						<div
							style="width:80px;height:80px;border-radius:50%;background:{data.primaryColor};display:inline-flex;align-items:center;justify-content:center;font-size:28px;font-weight:700;color:#fff;"
						>
							{initials}
						</div>
					{/if}
				...
				{#if data.logoUrl || Object.values(data.social).some(Boolean)}
					<div
						style="margin-top:10px;display:flex;align-items:center;justify-content:center;gap:12px;"
					>
						{#if data.logoUrl}
							<img
								src={data.logoUrl}
				...
				{#if data.disclaimer}
					<div
						style="margin-top:12px;border-top:1px solid {s.borderColor};padding-top:8px;font-size:{fs -
							3}px;color:{s.disclaimerColor};max-width:400px;white-space:pre-line;overflow-wrap:anywhere;"
					>
						{data.disclaimer}
					</div>
				{/if}
